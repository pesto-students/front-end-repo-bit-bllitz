   ...
   async generateOrder(state, action) {
        try {
            ...
            // Implement this function
             ...
             const { orderitemdata, ordererror } =  supabase.from('order_items').insert([
              { order_id:orderId,food_id: item.id, quantity: 1 ,
            ...
            const { data, error } = await supabase.from('orders').insert([
              { order_id:orderId, waiter_id: customerId, total_amount: totalAmount ,
